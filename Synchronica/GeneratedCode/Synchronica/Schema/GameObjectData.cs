...
public sealed class GameObjectData : Table {
  ...
  public VariableData GetProperties(int j) { return GetProperties(new VariableData(), j); }
  public VariableData GetProperties(VariableData obj, int j) { int o = __offset(6); return o != 0 ? obj.__init(__indirect(__vector(o) + j * 4), bb) : null; }
  ...
  public static VectorOffset CreatePropertiesVector(FlatBufferBuilder builder, Offset<VariableData>[] data) { builder.StartVector(4, data.Length, 4); for (int i = data.Length - 1; i >= 0; i--) builder.AddOffset(data[i].Value); return builder.EndVector(); }
